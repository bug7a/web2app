...
sideBar.create = function() {
    ...
    if (settings.showLoginViewButtonOnSideBar == 1) {
...
sideBar.selectItem = function(item) {
    ...
    sideBar.onSelectionChangeFunc(item.itemId); //item, item.itemI

    */

    item.boxBackground.dontMotion();
    item.boxBackground.color = "white";
    item.boxBackground.border = 1;
    item.boxBackground.withMotion(function(self) {
